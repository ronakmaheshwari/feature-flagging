...
export const addWhitelistService = async (
    adminId: string,
    flagId: string,
    userId: string,
) => {
    ...
    const alreadyWhitelisted = whitelist.some((x) => x.userId === userId);
    if (alreadyWhitelisted) {
        return { errorCode: 409, success: false, message: "The given user is already whitelisted" }
    }

    const updatedWhitelist = [...whitelist, { userId }];
    ...
    await db.$transaction(async (tx: Prisma.TransactionClient) => {
        ...
        await tx.feature_Flag_Audit.create({
            data: {
                flagId: flag.id,
                old_value: { whitelist: whitelist as unknown as Prisma.InputJsonValue } as Prisma.InputJsonObject,
                new_value: { whitelist: updatedWhitelist as unknown as Prisma.InputJsonValue } as Prisma.InputJsonObject,
                updatedBy: adminId
            }
        })
    })
    ...
    return {
        errorCode: 200,
        success: true,
        message: `The given user ${findUser.email} was added in whitelist`
    }
}